...
class FastSearchAdapter:
    ...
    def search_records(self, book_name: str, query: str = None, filters: Dict = None, limit: int = 100):
        ...
        if query:
            if len(query.strip()) < self.MIN_SEARCH_LENGTH:
                raise ValueError(f"Search query must be at least {self.MIN_SEARCH_LENGTH} characters long")
